...
fmask       = [filedir filesep 'nodif_brain_mask.nii.gz'];
...
DATAOUT.Rgfa = DATAOUT.Rstd ./ DATAOUT.Rrms;
DATAOUT.Rgfa(isnan(DATAOUT.Rgfa))=0;
DATAOUT.Rgfa(isinf(DATAOUT.Rgfa))=0;

DATAOUT.MeanGFA = mean(DATAOUT.Rgfa,4);
...
fprintf('%.1f seconds | %d voxels\n', DATAOUT.recon_elap_time, DATAOUT.recon_vox_count);
...
% fafile='/autofs/space/nyx_002/users/rjones/47t/20220318_162703_MF275_2022_03_18_1_1/ed_proc/dmri/lowb.nii.gz';
...
filelist = {'Rvec','Rstd','HWHMmin_vec','HWHMmax_vec','Rrms','Rgfa'};
for f=1:length(filelist)
    F = filelist{f};
    eval(['curr=DATAOUT.' F ';']);
    outfile = [niidir filesep F '.nii.gz'];
