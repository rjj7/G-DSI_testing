...
ReconZDPs               = false;    %recon zero-displacement maps (RTAP, RTPP)
...
% %   inputs/paths
% filedir      = '/autofs/space/nyx_002/users/rjones/47t/20220318_162703_MF275_2022_03_18_1_1/ed_proc/dmri';
% 
% fdwi        = [filedir filesep 'data.nii'];
% fmask       = [filedir filesep 'slices_mask.nii.gz'];
...
% if ~exist(outdir,'dir'), mkdir(outdir); fprintf(' making outdir: %s\n',outdir); end


big_delta   = 19.192 * 1e-3;    %ms
small_delta = 15.0 * 1e-3;    %ms



%   inputs/paths
froot      = '/autofs/space/nyx_002/users/rjones/47t/20220318_162703_MF275_2022_03_18_1_1/ed_proc/dmri';
filedir = [froot filesep 'z'];
fslices = [41 49 60];

for fs=1:length(fslices)

    currslice = fslices(fs);
    currslicepad = sprintf('%04d',currslice);
    
    fdwi        = [filedir filesep 'data' currslicepad '.nii.gz'];
    fmask       = [filedir filesep 'mask' currslicepad '.nii.gz'];
    fbvec       = [froot filesep 'bvecs'];
    fbval       = [froot filesep 'bvals'];
    flowb       = [froot filesep 'lowb.nii.gz']; %dtroot filesep 'preproc' filesep 'lowb_eddy_mean.nii.gz'];
    ...
    % Evenly spaced points b/w 0 and 1
    rs = linspace(0, 1, nr)';  
    % Evenly spaced points b/w 0 and MDD water
    dispvec  = linspace(0, mdd_water, length(rs));
    % Step size in radial direction
    r_step_size           = dispvec(2)-dispvec(1);
    % higher-order displacement vectors
    dispvec2 = dispvec.^2;
    dispvec4 = dispvec.^4;
