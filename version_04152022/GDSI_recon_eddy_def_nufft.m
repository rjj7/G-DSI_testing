function [ DATAOUT ] = GDSI_recon_eddy_def_nufft( dtroot, filedir ) %( fconfig, verbose )
% %   inputs/paths
...
%%%% FOR SHELLED DATA (qvol , q dens samp corr_
...
disp(' --- Finished voxel-wise reconstruction --- ');
% ftoc=toc(ftic);
...
if 0 == 1
    ...
    lb = prctile(b(mask(:,:,35)==1),[0 50]);
    b_=(b-lb(1)+(lb(1)/40))/(lb(2)-lb(1)); b_(b_>1)=1; b_(b_<0)=0;

    dec_hwhm_min = abs(a).*repmat(b_,1,1,3);
    ...
    ld = prctile(d(mask(:,:,35)==1),[10 80]);
    d_=(d-ld(1))/(ld(2)-ld(1)); d_(d_>1)=1; d_(d_<0)=0;

    dec_hwhm_max = abs(c).*repmat(d_,1,1,3);
