function [ DATAOUT ] = script_GDSI_recon_test( fdir, odir, isshelled, verbose ) %( fconfig, verbose )
% 
% GDSI reconstruction of DSI(grid) or Multi-shell diffusion data
%
% original script by (c) Qiyuan Tian, Stanford RSL
% Modified by RJ, Feb 2021 - Feb 2022
%
%
%  - fdir contains data.nii.gz, [nodif_]brain_mask.nii.gz, bvals, bvecs
%  - odir is output directory
%  - If data is multishell, isshelled should be true
%
...
%   inputs/paths
fdwi = [fdir filesep 'data.nii.gz'];
fmask = [fdir filesep 'brain_mask.nii.gz'];
fbval = [fdir filesep 'bvals'];
fbvec = [fdir filesep 'bvecs'];

% odir = [fdir filesep 'gdsi'];
if ~exist(odir,'dir'), mkdir(odir); end

%   params for gdsi (acq-specific)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Deal with multiple b=0 volumes
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     qvol = shelldens(DATAIN.bvals);
    ...
    qvol = ones(size(DATAIN.bvals));
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% MDD water (for PDF FOV)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%    - CS_DSI_100121:  Delta/delta = 25.5/10.8 ms --> MDDwater = 18.12 um    
...
%                 DATAOUT.RmsEAP(ix,iy,iz)           = rms(Rmatrix(:));
...
%                 (KLDiv(mtmp(1:50,:),isomtx(1:50,:)));   %compute KL divergence
                ...
                DATAOUT.KLDg(ix,iy,iz)   = (KLDiv(rtmp(1:50)',gaussmtx(1:50)'));   %compute KL divergence
                DATAOUT.KLDi(ix,iy,iz)   = (KLDiv(rtmp(1:50)',isomtx(1:50)'));   %compute KL divergence
...
foutmat = [odir filesep 'gdsi_vol_recon_prof+scal.mat'];

fprintf('Saving recon to file:\n %s\n',foutmat);
save(foutmat, 'DATAOUT', 'lowb', ... %'INFO', 'ACQ',...
   'pdf_dirs', 'dispvec', ...
...
%%%%%%%%%%%%
%%%% QUICK CHECK OF TISSUE PROFILES
%%%%%%%%%%%%
...
filelist = {'RTOP','MeanGA','MeanKurt','KLDg',...
            'KLDi','NLrms','EntropyEAP','NG',...
...
disp(' ');
disp(' - nii volumes saved, for scalars + profiles - ');
disp(' ');
disp(' -- NOW (FINALLY) ALL DONE :) -- ');
disp(' ');
