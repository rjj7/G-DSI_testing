function [ DATAOUT, DATAODF ] = allocate_gdsi_v5( voldims, nr, xflags )
%
% [ DATAOUT, DATAODF ] = allocate_gdsi_v5( voldims, nr, nr_odf )
% 

if nargin<2
    disp('USAGE: [ DATAOUT, DATAODF ] = allocate_gdsi_v3( voldims, nr[, flagodf] )');
    return
end
...
% Mean EAP profile (spherically averaged)
DATAOUT.RProfile        = zeros([voldims nr]);
% Generalized anisotropy profile - EAP (spherically averaged) 
DATAOUT.GAProfile        = zeros([voldims nr]);
% Generalized anisotropy profile - PDF (spherically averaged) 
DATAOUT.GAProfile_norm        = zeros([voldims nr]);
...
DATAOUT.RTOP_norm = zeros(voldims);
